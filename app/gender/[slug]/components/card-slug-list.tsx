                ...
                data.map((item) => {
                    return (
                        <div key={item.rank} className="flex gap-4 py-4 pl-6">
                            <div className="w-40 h-[167px]">
                                <Image className="w-32 h-[167px]" src={item.book_image} alt={item.title} width={120} height={120} />
                            </div>
                            <div className="flex flex-col gap-2">
                                <div className='flex flex-col gap-1 lg:flex-row'>
                                    <h2 className="text-[#0E1337] text-base font-bold leading-snug">{item.title}</h2>
                                    <p className="text-{#454A67] font-normal text-sm leading-normal">{item.contributor}</p>
                                </div>
