...
const GenderLine: React.FC<GenderLineProps> = ({ data }) => {
    return (
        <>
            {data?.map((item) => (
                <div key={item.list_name} className="grid grid-cols-1 lg:grid-cols-3 gap-1 my-5 lg:gap-4 lg:my-10 items-center lg:pb-4">
                    ...
                    <p className="lg:text-center text-xs font-normal text-[#454A67]">Última publicação em {new Date(item.newest_published_date).toLocaleDateString('pt-BR')}</p>
                    <p className="lg:text-end text-xs font-normal text-[#454A67]">Publicação mais antiga {new Date(item.newest_published_date).toLocaleDateString('pt-BR')}</p>
                </div>
            ))}
        </>
    )
